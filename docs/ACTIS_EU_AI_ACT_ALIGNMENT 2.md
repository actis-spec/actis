...
ACTIS provides a standardized, verifiable evidence format for commitment-grade agent events. This document maps ACTIS's existing capabilities to Article 12's specific requirements, making explicit what ACTIS covers, what it does not, and what supplementary practices are needed.
...
| **§2: Traceable to risk situations** | `failure_event` with `code`, `stage`, `fault_domain`, `terminality` | ✅ Full | Failure taxonomy is currently Pact-specific (`PACT-XXX` codes). | Define an ACTIS-neutral error code namespace for cross-vendor risk identification. |
...
## 3. Compliance Memo Statement

The following statement may be included in an organization's AI system technical documentation (Article 11):

> **ACTIS Compliance Note**
>
> This AI system generates evidence bundles conforming to ACTIS v1.0 (Audit-Compliant Transaction Integrity Standard) for all commitment-grade agent events. ACTIS evidence provides:
>
...
> - **Substantial modification detection** through cryptographic hash chains that detect any post-hoc modification to recorded events (Article 12 §2).
> - **Integrity verification** via an independent, vendor-neutral verification tool that validates evidence without access to proprietary system internals.
>
> Evidence bundles conform to the schema defined in `actis-transcript/1.0` and are verifiable using any ACTIS v1.0-compatible verifier. Bundles are retained for [RETENTION PERIOD] in immutable storage in accordance with Article 12 §3.
>
> For sub-decision telemetry not covered by ACTIS (e.g., intermediate reasoning, tool call outcomes), this system uses [OBSERVABILITY SYSTEM] with a retention period of [RETENTION PERIOD].
>
> *This note is provided for Article 11 technical documentation purposes and does not constitute legal advice or a compliance certification.*
...
| Pact-specific failure codes (`PACT-XXX`) | Low | Define ACTIS-neutral error code namespace in semantic conventions v1.1 |
