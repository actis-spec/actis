...
ACTIS is stewarded by the ACTIS Maintainers. See [GOVERNANCE.md](./GOVERNANCE.md) and [MAINTAINERS](./MAINTAINERS) for stewardship and the transition to independent governance.
...
4. **Run the conformance harness** — From `actis/test-vectors/`, run `./run_conformance.sh` with `ACTIS_VERIFIER_CMD` set to your verifier. The harness validates your implementation against the tv-001..tv-011 corpus.
...
├── test-vectors/      ← tv-001..tv-011 corpus, expected_results.json, run_conformance.sh
└── ...
```
